...
public class ItemsController {
    ...
    public String viewItems(@RequestParam("itemId") int id, ModelMap model){
        itemID = id;
        List<RestaurantModel> restaurants = itemsSer.viewRestaurantInItems(id);
        restaurants.forEach(i -> System.out.println(i.getName()));
        model.put("restaurants",restaurants);
        return "restaurantsInItems";
    }
    @GetMapping("/restautantRemove")
    public String restaurantRemove(@RequestParam("itemId") int itemId){
        itemsSer.restautantRemove(restaurantID,itemId);
        return "redirect:/item/viewItems?itemId=" + itemID;
    }

    @GetMapping("/update")
    private String update(@RequestParam("itemId") int id, ModelMap model){
        ItemsModel itemsModel = itemsSer.getItem(id);
        model.put("itemsModel",itemsModel);
        return "itemUpdate";
    }
    @PostMapping("/updated")
    private String itemUpdated(@ModelAttribute("itemsModel") ItemsModel itemsModel){
        itemsSer.itemUpdate(itemsModel);
        return "redirect:/item/view";
    }
}
