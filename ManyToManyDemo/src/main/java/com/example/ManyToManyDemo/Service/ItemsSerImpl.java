...
public class ItemsSerImpl implements ItemSerInterface {
    ...
    public List<RestaurantModel> viewRestaurantInItems(int id){
       ItemsEntity itemsEntity = itemsRepo.findById(id).get();
        Set<RestaurantEntity> restaurantEntities= itemsEntity.getRestaurantEntity();
    ...
    public void restautantRemove(int restaurantID, int itemID){
        RestaurantEntity restaurantEntity = restaurantRepo.findById(restaurantID).get();
        ItemsEntity itemsEntity = itemsRepo.findById(itemID).get();
        itemsEntity.getRestaurantEntity().remove(restaurantEntity);
        itemsRepo.save(itemsEntity);
    }
